...
describe("slugifyBranchComponent", () => {
	test("lowercases and replaces spaces with hyphens", () => {
		expect(slugifyBranchComponent("Customer Onboarding")).toBe("customer-onboarding");
	});

	test("replaces slashes and special characters", () => {
		expect(slugifyBranchComponent("OAuth / SSO polish")).toBe("oauth-sso-polish");
	});
	...
	test("handles mixed special characters", () => {
		expect(slugifyBranchComponent("feat: add (new) thing!")).toBe("feat-add-new-thing");
	});
