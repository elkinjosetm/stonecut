...
describe("setupSkills", () => {
  test("creates symlinks", () => {
    ...
    for (const name of SKILL_NAMES) {
      ...
      expect(realpathSync(link)).toBe(
        realpathSync(join(getSkillsSourceDir(), name)),
      );
    }
  });
  ...
  test("prints linked skills", () => {
    ...
    for (const name of SKILL_NAMES) {
      expect(result.messages.some((m) => m.includes(`Linked ${name}`))).toBe(
        true,
      );
    }
  });
  ...
  test("idempotent — second run skips silently", () => {
    ...
    for (const name of SKILL_NAMES) {
      expect(result2.messages.some((m) => m.includes(`Linked ${name}`))).toBe(
        false,
      );
    }
...
describe("setupSkills conflicts", () => {
  test("warns symlink pointing elsewhere", () => {
    ...
    const result = setupSkills(root);
    expect(
      result.warnings.some((w) => w.includes("already exists as symlink")),
    ).toBe(true);
  ...
  test("warns regular directory", () => {
    ...
    const result = setupSkills(root);
    expect(result.warnings.some((w) => w.includes("not a symlink"))).toBe(
      true,
    );
...
describe("removeSkills", () => {
  test("removes forge symlinks", () => {
    ...
    for (const name of SKILL_NAMES) {
      ...
      expect(result.messages.some((m) => m.includes(`Removed ${name}`))).toBe(
        true,
      );
    }
  });
...
describe("legacy skill migration", () => {
  test("setup-skills removes legacy forge:* symlinks", () => {
    ...
    expect(
      result.messages.some((m) => m.includes("legacy skill link")),
    ).toBe(true);
  });
