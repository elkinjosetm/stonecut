...
function withMockSpawnSync(
  handler: (cmd: string[]) => SpawnSyncResult,
): void {
...
function makeSource(): GitHubSource {
  withMockSpawnSync((cmd) => {
    ...
    if (bin === "git" && cmd[1] === "remote")
      return ok("git@github.com:owner/repo.git\n");
    if (bin === "gh" && cmd[1] === "issue" && cmd[2] === "view")
      return ok("prd\n");
...
describe("gh CLI validation", () => {
  test("error when gh not installed", () => {
    ...
    expect(() => GitHubSource.validateGhCli()).toThrow(
...
describe("PRD validation", () => {
  ...
  test("error when issue lacks prd label", () => {
    ...
    expect(() => source.validatePrd()).toThrow(
      "does not have the 'prd' label",
    );
  });
