...
export class LocalSource implements Source<Issue> {
  ...
  private validate(): void {
    ...
    if (!existsSync(join(this.specDir, "prd.md")) || !statSync(join(this.specDir, "prd.md")).isFile()) {
      throw new Error(`Error: ${this.specDir}/prd.md not found`);
    }
    if (!existsSync(join(this.specDir, "issues")) || !statSync(join(this.specDir, "issues")).isDirectory()) {
  ...
  async completeIssue(issue: Issue): Promise<void> {
    ...
    appendFileSync(join(this.specDir, "progress.txt"), `${timestamp} — Issue ${issue.number} complete: ${issue.filename}\n`);
  }
}
