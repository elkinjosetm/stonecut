...
export class GitHubSource implements Source<GitHubIssue> {
  ...
  static getOwnerRepo(): [string, string] {
    ...
    throw new Error(
      `Error: could not parse owner/repo from remote URL: ${url}`,
    );
  }
  ...
  validatePrd(): void {
    ...
    if (!labels.includes("prd")) {
      throw new Error(
        `Error: Issue #${this.prdNumber} does not have the 'prd' label.`,
      );
    }
  }
  ...
  getPrd(): GitHubPrd {
    ...
    if (result.exitCode !== 0) {
      throw new Error(
        `Error: failed to fetch PRD issue #${this.prdNumber}.`,
      );
    }
...
query($owner: String!, $repo: String!, $number: Int!) {
  repository(owner: $owner, name: $repo) {
    ...
    if (result.exitCode !== 0) {
      throw new Error(
        `Error fetching sub-issues: ${result.stderr.trim()}`,
      );
    }
    const data = JSON.parse(result.stdout);
  ...
  async completeIssue(issue: GitHubIssue): Promise<void> {
    ...
    if (result.exitCode !== 0) {
      throw new Error(
        `Error: failed to close issue #${issue.number}: ${result.stderr.trim()}`,
      );
    }
  }
}
